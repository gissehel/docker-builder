...
create_dockerfile() {
    ...
    image_name="$(echo "${image_prefix}${id}" | sed -e 's|\\/|\/|g;')"
    echo "" >> "${dockerfilename}"
    echo "ARG BUILD_DATE" >> "${dockerfilename}"
    echo "ARG VCS_REF" >> "${dockerfilename}"
    echo "" >> "${dockerfilename}"
    echo "LABEL \\" >> "${dockerfilename}"
    echo '      org.opencontainers.image.created="${BUILD_DATE}" \' >> "${dockerfilename}"
    echo '      org.opencontainers.image.authors="'"${authors}"'" \' >> "${dockerfilename}"
    echo '      org.opencontainers.image.url="'"${project_url}"'" \' >> "${dockerfilename}"
    # echo '      org.opencontainers.image.documentation="'"${documentation}"'" \' >> "${dockerfilename}"
    echo '      org.opencontainers.image.source="'"${project_url}"'" \' >> "${dockerfilename}"
    echo '      org.opencontainers.image.version="'"${VERSION}"'-${VCS_REF}" \' >> "${dockerfilename}"
    echo '      org.opencontainers.image.revision="${VCS_REF}" \' >> "${dockerfilename}"
    echo '      org.opencontainers.image.vendor="'"${vendor}"'" \' >> "${dockerfilename}"
    [ -n "${licenses}"  ] && echo '      org.opencontainers.image.licenses="'"${licenses}"'" \' >> "${dockerfilename}"
    echo '      org.opencontainers.image.ref.name="'"${image_name}"'" \' >> "${dockerfilename}"
    echo '      org.opencontainers.image.title="'"${id}"'" \' >> "${dockerfilename}"
    echo '      org.opencontainers.image.description="Image for '"${id}"'" \' >> "${dockerfilename}"
